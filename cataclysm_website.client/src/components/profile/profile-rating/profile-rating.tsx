...
>();
  ...
  function getBracketColor(bracket: string, current_rating: number): string | undefined {
...
function getTitle(bracket: string, current_rating: number): string | undefined {
  const bracketRewards = rewards?.filter(r => {
    ...
    for (const bracketReward of bracketRewards ?? [])  {
        if (current_rating >= bracketReward.rating_cutoff)
          title = bracketReward.achievement?.name?.replace(' - Season 9', '').replace(': Season 9', '').replace('[DNT] ', '') ?? ""
    }
return title;
}

function getCardBorder(bracket: string, current_rating: number): string | undefined {
  const title = getTitle(bracket, current_rating)
  ...
  function getRatingCard(BracketStatistics :Dragonblight.CharacterPvpBracketStatistics | null, index: number) : ReactElement { //connects specific bracket data into card, set to type ReactElement with tsx code inside return
    return (
      <div className="pb-10 rating-card" key={"GetRating" + index}>
        <Card className={getCardBorder(BracketStatistics?.bracket?.type ?? "", BracketStatistics?.rating ?? 0) + 
          " " + (index == 0 || index == 2 ? "mt-5 mb-3 h-26 mobile-padding rating-padding" : "mt-5 mb-3 h-26 rating-padding")}>
            <div>
            <div className={getBracketColor(BracketStatistics?.bracket?.type ?? "", BracketStatistics?.rating ?? 0) + " " + ("font-bold text-3xl flex justify-center")}>{BracketStatistics?.rating ? BracketStatistics?.rating : "0"}</div>
              </div>
              <Heading size="3" className="text-center" > {index == 0 ? "2v2" : ""} 
              {index == 1 ? "3v3" : ""}
              {index == 2 ? "5v5" : ""}
              {index == 3 ? "RBG" : ""}
              </Heading>
        ...
        <div className="flex justify-center text-sm">
                <span className="text-green-300">{BracketStatistics?.season_match_statistics?.won}</span> &nbsp;-&nbsp; <span className="text-red-300">{BracketStatistics?.season_match_statistics?.lost}</span>
              </div>
        <span className="text-xs italic justify-center">
            Predicted Title: {getTitle(BracketStatistics?.bracket?.type ?? "", BracketStatistics?.rating ?? 0)}
            </span>
      </div>
    );
  }
