...
export class PvpLeaderboardClient {
    ...
    getSeasonStart(region: string | undefined): Promise<Date | null> {
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetSeasonStart(_response);
        });
    }

    protected processGetSeasonStart(response: Response): Promise<Date | null> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<Date | null>(null as any);
    }
